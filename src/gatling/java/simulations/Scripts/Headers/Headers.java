package simulations.Scripts.Headers;

import simulations.Scripts.Utilities.AppConfig;
import java.util.Collections;
import java.util.HashMap;
import java.util.Map;

/**
 * Manages HTTP headers for different types of requests in the application.
 * This class provides predefined header sets for various types of HTTP requests
 * and utilities for header manipulation.
 */
public final class Headers {

    private Headers() {
        throw new AssertionError("Utility class - do not instantiate");
    }

    // Common header values
    private static final String BROWSER_UA = 
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/143.0.0.0 Safari/537.36";
    
    private static final String SEC_CH_UA = "Google Chrome\";v=\"143\", \"Chromium\";v=\"143\", \"Not A(Brand\";v=\"24";
    private static final String SEC_CH_UA_QUOTED = "\"Google Chrome\";v=\"143\", \"Chromium\";v=\"143\", \"Not A(Brand\";v=\"24\"";
    private static final String SEC_CH_UA_MOBILE = "?0";
    private static final String SEC_CH_UA_PLATFORM = "Windows";
    private static final String SEC_CH_UA_PLATFORM_QUOTED = "\"Windows\"";
    
    private static final String ACCEPT_HTML = 
        "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7";
    private static final String ACCEPT_JSON = "application/json";
    private static final String ACCEPT_JSON_PLAIN = "application/json, text/plain, */*";
    
    private static final String OPAL_FRONTEND_URL = "https://opal-frontend.test.platform.hmcts.net/";
    private static final String OPAL_FRONTEND = "https://opal-frontend.test.platform.hmcts.net";

    /**
     * Gets a map of headers for a specific header type.
     * 
     * @param headerTypeValue The header type value from HeaderType enum
     * @return An immutable map of headers for the specified type
     */
    public static Map<String, String> getHeaders(int headerTypeValue) {
        HeaderType headerType = HeaderType.fromValue(headerTypeValue);
        Map<String, String> headers = new HashMap<>();

        switch (headerType) {
            case TEST_1:
                addBrowserNavigationHeaders(headers);
                headers.put("Referer", OPAL_FRONTEND_URL);
                break;
            
            case TEST_2:
                addMSOnlineHeaders(headers);
                break;

            case TEST_3:
                headers.put("Content-Type", "application/x-www-form-urlencoded");
                headers.put("Origin", "https://login.microsoftonline.com");
                headers.put("Referer", "https://login.microsoftonline.com/" + AppConfig.TenantConfig.AAD_TENANT_ID + "/oauth2/v2.0/authorize");
                headers.put("Accept", ACCEPT_HTML);
                headers.put("User-Agent", BROWSER_UA);
                headers.put("sec-ch-ua", SEC_CH_UA_QUOTED);
                headers.put("sec-ch-ua-mobile", SEC_CH_UA_MOBILE);
                headers.put("sec-ch-ua-platform", SEC_CH_UA_PLATFORM_QUOTED);
                break;

            case TEST_4:
                addBrowserNavigationHeaders(headers);
                headers.put("Cache-Control", "max-age=0");
                headers.put("Origin", "https://login.microsoftonline.com");
                headers.put("Sec-Fetch-Site", "cross-site");
                break;

            case TEST_5:
                addCORSHeaders(headers);
                headers.put("Accept", ACCEPT_JSON_PLAIN);
                headers.put("Cache-Control", "no-cache");
                headers.put("Expires", "0");
                headers.put("Pragma", "no-cache");
                break;

            case TEST_6:
                addCORSHeaders(headers);
                headers.put("Accept", ACCEPT_JSON_PLAIN);
                headers.put("If-None-Match", "W/\"35-pzeKyq0NE9c2fLCr1PPYqFCAl+w\"");
                break;

            case TEST_7:
                addCORSHeaders(headers);
                headers.put("Accept", ACCEPT_JSON_PLAIN);
                break;

            case TEST_8:
                headers.put("Connection", "keep-alive");
                headers.put("Upgrade-Insecure-Requests", "1");
                headers.put("User-Agent", BROWSER_UA);
                headers.put("Accept", ACCEPT_HTML);
                headers.put("Accept-Language", "en-US,en;q=0.9");
                headers.put("Referer", OPAL_FRONTEND_URL);
                addBrowserSecurityHeaders(headers);
                break;

            case TEST_9:
                addBrowserNavigationHeaders(headers);
                headers.put("Sec-Fetch-Site", "cross-site");
                headers.put("Accept", ACCEPT_HTML);
                break;

            case TEST_10:
                addBrowserNavigationHeaders(headers);
                headers.put("Sec-Fetch-User", "?1");
                headers.put("Accept", ACCEPT_HTML);
                headers.put("Accept-Encoding", "gzip, deflate, br, zstd");
                headers.put("Accept-Language", "en-US,en;q=0.9");
                headers.put("Connection", "keep-alive");
                break;

            case TEST_11:
                addCORSHeaders(headers);
                headers.put("cache-control", "no-cache");
                headers.put("expires", "0");
                headers.put("pragma", "no-cache");
                break;

            case TEST_12:
                addCORSHeaders(headers);
                break;

            case TEST_13:
                addCORSHeaders(headers);
                headers.put("cache-control", "no-cache");
                headers.put("expires", "0");
                headers.put("pragma", "no-cache");
                break;

            case TEST_14:
                addAPIHeaders(headers);
                break;

            case TEST_15:
                addAPIHeaders(headers);
                headers.put("Cache-Control", "max-age=0");
                headers.put("if-match", "0");
                break;
        }

        return Collections.unmodifiableMap(headers);
    }

    /**
     * Adds common browser navigation security headers
     */
    private static void addBrowserNavigationHeaders(Map<String, String> headers) {
        headers.put("Sec-Fetch-Dest", "document");
        headers.put("Sec-Fetch-Mode", "navigate");
        headers.put("Sec-Fetch-Site", "same-origin");
        headers.put("Upgrade-Insecure-Requests", "1");
        addBrowserSecurityHeaders(headers);
    }

    /**
     * Adds common browser security headers
     */
    private static void addBrowserSecurityHeaders(Map<String, String> headers) {
        headers.put("User-Agent", BROWSER_UA);
        headers.put("sec-ch-ua", SEC_CH_UA_QUOTED);
        headers.put("sec-ch-ua-mobile", SEC_CH_UA_MOBILE);
        headers.put("sec-ch-ua-platform", SEC_CH_UA_PLATFORM_QUOTED);
    }

    /**
     * Adds common CORS headers
     */
    private static void addCORSHeaders(Map<String, String> headers) {
        headers.put("Sec-Fetch-Dest", "empty");
        headers.put("Sec-Fetch-Mode", "cors");
        headers.put("Sec-Fetch-Site", "same-origin");
        headers.put("sec-ch-ua", SEC_CH_UA);
        headers.put("sec-ch-ua-mobile", SEC_CH_UA_MOBILE);
        headers.put("sec-ch-ua-platform", SEC_CH_UA_PLATFORM);
    }

    /**
     * Adds Microsoft Online headers
     */
    private static void addMSOnlineHeaders(Map<String, String> headers) {
        headers.put("Accept", ACCEPT_JSON);
        headers.put("Content-type", "application/json; charset=UTF-8");
        headers.put("Origin", "https://login.microsoftonline.com");
        headers.put("Sec-Fetch-Dest", "empty");
        headers.put("Sec-Fetch-Mode", "cors");
        headers.put("Sec-Fetch-Site", "same-origin");
        headers.put("canary", "#{apiCanary}");
        headers.put("client-request-id", "#{getClientRequestId}");
        headers.put("hpgact", "1800");
        headers.put("hpgid", "1104");
        headers.put("hpgrequestid", "#{sessionId}");
        headers.put("sec-ch-ua", SEC_CH_UA);
        headers.put("sec-ch-ua-mobile", SEC_CH_UA_MOBILE);
        headers.put("sec-ch-ua-platform", SEC_CH_UA_PLATFORM);
    }

    /**
     * Adds OPAL API headers
     */
    private static void addAPIHeaders(Map<String, String> headers) {
        headers.put("Origin", OPAL_FRONTEND);
        headers.put("Sec-Fetch-Dest", "empty");
        headers.put("Sec-Fetch-Mode", "cors");
        headers.put("Sec-Fetch-Site", "same-origin");
        headers.put("content-type", "application/json");
        headers.put("sec-ch-ua", SEC_CH_UA);
        headers.put("sec-ch-ua-mobile", SEC_CH_UA_MOBILE);
        headers.put("sec-ch-ua-platform", SEC_CH_UA_PLATFORM);
    }
}